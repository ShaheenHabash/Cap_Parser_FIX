...
namespace CAP_Parser_FIX.Tools
{
    public class MessageConstructor
    {
        ...
        public string SequenceResetMessage(SessionQualifier qualifier, int messageSequenceNumber, int rejectSequenceNumber)
        {
            ...
            var header = ConstructHeader(qualifier, SessionMessageCode(SessionMessageType.SequenceReset), messageSequenceNumber, body.ToString());
            var headerAndBody = header + body;
            var trailer = ConstructTrailer(headerAndBody);
            var headerAndMessageAndTrailer = header + body + trailer;
            return headerAndMessageAndTrailer.Replace("|", "\u0001");
        ...
        /// <param name="minQty">Minimum quantity of an order to be executed.</param>
        /// <param name="maxFloor">Maximum quantity within an order to be shown at any given time.</param>
        /// <param name="account">Unique identifier for the order, allocated by the client.</param>
        /// <param name="clientId">Unique identifier for the order, allocated by the client.</param>
        /// <param name="traderId">Unique identifier for the order, allocated by the client.</param>
        ...
        /// <returns></returns>
        public string NewOrderSingleMessage(SessionQualifier qualifier, int messageSequenceNumber, string stopPx, string minQty, string maxFloor, string account, string clientId, string traderId, string orderID, string symbol, int side, int orderQuantity, int orderType, string timeInForce, decimal price, string expireTime)
        {
            var body = new StringBuilder();
            // HandlInst.
            body.Append("21=" + "1" + "|");
            // Client generated request time.
            body.Append("60=" + DateTime.UtcNow.ToString("yyyyMMdd-HH:mm:ss") + "|");
            //Unique identifier for the order, allocated by the client.
            body.Append("11=" + orderID + "|");
            //SecurityIDSource = 4 (ISIN number).
            body.Append("48=" + symbol + "|");
            //SecurityIDSource = 4 (ISIN number).
            body.Append("22=" + "4" + "|");
            //Instrument identificators are provided by Spotware.
            body.Append("55=" + symbol + "|");
            ...
            //ClientID.
            body.Append("109=" + clientId + "|");
            //ID of the Trader issuing the order.
            body.Append("9947=" + traderId + "|");
            //Text.
            body.Append("58=" + "Test buy ASE" + "|");

            if (expireTime.Nvl("").Trim() != "")
            {
            ...
            {
                // StopPx
                body.Append("99=" + stopPx + "|");
            }
            if (minQty.Nvl("").Trim() != "")
            {
                // MinQty
                body.Append("110=" + minQty + "|");
            }
            if (maxFloor.Nvl("").Trim() != "")
            {
                // MaxFloor
        ...
        /// (see TimeInForce: GTD).</param>
        /// <returns></returns>
        public string OrderCancelRequestMessage(SessionQualifier qualifier, int messageSequenceNumber,string origOrderID, string orderID, string symbol, int side, int orderQuantity)
        {
            var body = new StringBuilder();
            //Unique identifier for the order, allocated by the client.
            body.Append("11=" + orderID + "|");
            ...
            body.Append("60=" + DateTime.UtcNow.ToString("yyyyMMdd-HH:mm:ss") + "|");
            //OrigOrderID
            body.Append("9945=" + origOrderID + "|");
            

            var header = ConstructHeader(qualifier, ApplicationMessageCode(ApplicationMessageType.OrderCancelRequest), messageSequenceNumber, body.ToString());
            var headerAndBody = header + body;
            var trailer = ConstructTrailer(headerAndBody);
            var headerAndMessageAndTrailer = header + body + trailer;
            return headerAndMessageAndTrailer.Replace("|", "\u0001");
            ...
            int messageSequenceNumber, string bodyMessage)
        {
            ...
            message.Append("56=" + qualifier.Nvl(0) + "|");
            // Message target. Valid value is "CSERVER"
            message.Append("57=" + _targetCompID + "|");
